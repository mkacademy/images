...
import { isServerId } from './sliceUtils';
import {
...
/**
 * Prefer an explicit convolution destination on redirectUrl.
 * Only fall back to `ldr` (often `/` loading) when the URL is not already a content route.
 * Used by continue-incognito to return straight to content.
 */
export const resolveLdrRedirectUrl = (url: string): string => {
  if (!url) return url;
  const pathOnly = url.split('?')[0];
  if (pathOnly.startsWith('/convolution/')) return url;
...
/**
 * After successful login, prefer the loading screen (`ldr`, typically `/?...`)
 * so Loading can enforce its min delay before navigating to convolution.
 */
export const resolveLoginLoadingRedirectUrl = (url: string): string => {
  ...
  // Convolution redirectUrl without ldr — rebuild a loading URL from its search.
  if (pathOnly.startsWith('/convolution/')) {
