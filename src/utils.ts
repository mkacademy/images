...
import { DescendentItem } from './types/cpanel';
...
export interface EntityPropertyMap {
  ...
  mockedData?: (metadatas: Metadata[], connections: string[]) => MockedDataReturn;
}
...
/** Editor CMS origin: local dev editor app, production cms host. */
export const getEditorUrl = (): string => {
...
export const Tree = {
  ...
  getProperty<K extends keyof EntityPropertyMap>(entity: string, propertyName: K): EntityPropertyMap[K] {
    ...
    if (this.entities && this.entities.length) {
      ...
      if (parent) return parent[propertyName] as EntityPropertyMap[K];
      return undefined;
