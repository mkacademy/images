...
} from '../../library/comments-api-body';
...
function bodyPathForRole(userRole: CommentItem['userRole']): CommentsBodyPath | null {
  if (userRole === 'b') return COMMENTS_BODY_PATHS.bossesInstructions;
  if (userRole === 'u') return COMMENTS_BODY_PATHS.underbossesInstructions;
  if (userRole === 'm') return COMMENTS_BODY_PATHS.minionsInstructions;
  return null;
}
...
export const fetchMessageComments = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const {
        session: { curMailer, curToken, fetchRole, isPrivate },
      } = state;
