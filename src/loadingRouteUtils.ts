...
export const resolveViewerDeepLinkSearch = (search: string): string => {
...
export const getDeepLinkTreeIds = (search: string): DeepLinkTreeIds => {
  const params = searchParamsFrom(resolveViewerDeepLinkSearch(search));
...
export const parseLoadingDeepLinkPairs = (search: string): LoadingDeepLinkPair[] => {
  const params = searchParamsFrom(resolveViewerDeepLinkSearch(search));
  return LOADING_DEEP_LINK_PAIRS.filter(
    (pair) => params.has(pair.zipper) && params.has(pair.webapp),
  );
};
...
export const primaryLoadingWebapp = (
  ...
  const { hasTutorial, hasCourse, hasQuiz } = parseLoadingTreeFlags(search);
